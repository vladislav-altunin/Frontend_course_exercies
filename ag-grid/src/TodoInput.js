...
function TodoList () {
    ...
    //Add task 
    const addTodo = () => { // is the event param necessary?
        setIsClicked(true);
    ...
    const deleteTodo = () => {
        if(gridRef.current.getSelectedNodes().length > 0) {
            setTodoList(todoList.filter( (todo, i) => i !== gridRef.current.getSelectedNodes()[0].childIndex));
        } else {
            alert('No row chosen!');
        }
    ...
    if(!isClicked || todoList.length === 0) {
        return (
            <div style={{height: '500px', width: '70%', margin: 'auto'}}>
                <input name="description" type="text" placeholder="Description" value={todo.description} onChange={inputChanged} />
                <input name="date" type="text" placeholder="Date" value={todo.date} onChange={inputChanged}/>
                <input name="priority" placeholder="Priority" type="text" value={todo.priority} onChange={inputChanged}/>
        ...
        return(
            <div style={{height: '500px', width: '70%'}}>
                ...
                <button onClick={addTodo}>Add</button>
                <button onClick={deleteTodo}>Delete</button>
                <TodoTable todoList={todoList} deleteTodo={deleteTodo} gridRef={gridRef} columns={columns}/>
            </div>
        )
    }
}
