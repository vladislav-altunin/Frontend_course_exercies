...
function TodoTable(props) {
  ...
  //Got sizeColumnsToFit() to work after defining a separate function
  const onGridReady = event => {
    props.gridRef.current = event.api;
  ...
  return (
    ...
    //div AG Grid inline-styles style={{height: '500px', width: '70%', margin: 'auto'}}
    <div
      style={{
        display: 'flex',
        alignItems: 'center',
    ...
    >
      ...
      >
        <AgGridReact
          ...
          onGridReady={onGridReady} //(4) onGridReady event prop is invoked when the grid is ready. It's used to set the reference to Grid's API
        ></AgGridReact>
      </div>
    </div>
  );
